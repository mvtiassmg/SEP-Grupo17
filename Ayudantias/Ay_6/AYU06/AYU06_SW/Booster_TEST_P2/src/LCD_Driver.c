...
/********************************************************************************
Esta función escribe el framebuffer en la pantalla LCD.
********************************************************************************/
void LCD_WriteBuffer(uint16_t *pBuffer, uint32_t NumPixels) {
    DC_HIGH();
    for(uint32_t i = 0; i < NumPixels; i++) {
        uint16_t color = pBuffer[i];
        SPI_Write(&SpiInstance, (color >> 8) & 0xFF);
        SPI_Write(&SpiInstance, color & 0xFF);
    }
}
