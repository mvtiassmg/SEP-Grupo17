...
#define TMRCTR_INTERRUPT_ID     XPAR_INTC_0_TMRCTR_0_VEC_ID   // <-- USAR VECTOR DEL TIMER 0
...
#define LED_CHANNEL             1          // Canal 1: LEDs (AXI_GPIO_0)
...
#define SWITCHES_CHANNEL        2          // Canal 2: switches (AXI_GPIO_0)
...
int main(void)
{
    // Inicializar GPIO (AXI_GPIO_0)
    XGpio_Initialize(&GPIO, XPAR_GPIO_0_DEVICE_ID);

    // Canal 1 como salida (todos los bits) -> LEDs
    ...
    // Configurar el AXI Timer 0 + interrupciones (parpadeo LEDs 0-3)
    ...
    // Inicializar segundo AXI Timer (AXI_TIMER_1) en modo PWM (LED verde RGB)
    PwmTimerInit();
                       ...
                       u8       TmrCtrNumber)
{
    ...
    // 3) Registrar nuestro handler de alto nivel (común para ambos timers internos)
    ...
    // 7) Arrancar ambos timers internos del AXI_TIMER_0
