 ...
 * Ajusta esto al nombre que te genere Vivado en xparameters.h
 * (búscalo como *_S00_AXI_BASEADDR)
 */
...
// Ahora el timer dispara cada 1 ms
#define TIMER_ISR_SEC       0.001f      // 1 ms
#define GAME_TICK_MS        40          // 40 ms por frame de juego
#define GAME_TICKS_PER_SEC  (1000 / GAME_TICK_MS)   // 25
...
volatile u32  isr_counter      = 0;   // Cuenta cuántas veces disparó el timer (1 ms)
volatile int  game_tick_div    = 0;   // Divide 1 ms -> 40 ms
...
/* ISR del Timer:
 * - Se llama cada 1 ms.
 * - Cada 40 ms levanta game_tick_flag para que el main avance la lógica.
...
static void GameTimer_ISR(void *CallBackRef, u8 TmrNum)
{
    ...
    if (XTmrCtr_IsExpired(InstancePtr, TmrNum)) {
        ...
        // Contador de ISRs (1 ms cada una)
        isr_counter++;
        ...
        // Divisor de 1 ms -> 40 ms para el juego
        game_tick_div++;
        if (game_tick_div >= GAME_TICK_MS) {   // 40 ISR de 1 ms = 40 ms
            game_tick_div = 0;
            game_tick_flag = 1;
        }
...
static int SetupInterruptSystem(void)
{
    ...
    u32 load_value = seconds_to_ticks(TIMER_ISR_SEC); // 1 ms
    xil_printf("[INT] Timer ticks por periodo: %lu (%.3f ms)\r\n",
               (unsigned long)load_value, TIMER_ISR_SEC * 1000.0f);
    ...
    xil_printf("[INT] Timer (1 ms) + GIC configurados OK\r\n");
...
int main(void)
{
    ...
    init_platform();
    xil_printf("\r\n--- INICIO SISTEMA (juego + canción con interrupciones) ---\r\n");
    ...
    /* 2. Configurar interrupciones (Timer 1 ms) */
    xil_printf("[INIT] Configurando sistema de interrupciones (Timer 1 ms)...\r\n");
    ...
    /* 3. Menú de control del jugador */
    Show_Menu();
    opt = inbyte();
    xil_printf("%c\r\n", opt);

    seed_val = 1000;  // Fijo, para debug más determinista
    ...
    if (opt_music == 's' || opt_music == 'S') {
        ...
        g_music_on = 1;                     // La ISR empezará a llamar Tetris_Tick_1ms
    } else {
    ...
    while (1) {
        ...
        /* === Esperar al tick de juego (cada 40 ms) === */
        if (!game_tick_flag) {
        ...
        if (tick_counter_1s >= GAME_TICKS_PER_SEC) {
            ...
            // Debug de frecuencia de ISR (debería ~1000 por segundo)
